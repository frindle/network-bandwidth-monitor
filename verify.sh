#!/usr/bin/env bash
# verify for: netmon-telegraf-metrics
...
echo "=== target parses ==="
if "$PY" -c "import ast,sys; ast.parse(open('app/metrics.py').read())"; then
  echo "  ok: app/metrics.py parses"
else
  echo "  FAIL: app/metrics.py does not parse"; fails=$((fails+1))
fi
